...
class ProductCreateEditViewModel (private val productRepository: ProductRepository, private val categoryRepository: CategoryRepository) : ViewModel(){
    ...
    fun updateProductWithImage(token: String, storeId: String, product : Product, categoryId: Int, withImage: Boolean){
        setLoading()
        if(withImage){
    ...
    fun validate(name : String, desc : String, price: Int?, quantity : Int?, isAvailableOnline : Boolean ,weight: Double?, categoryId : Int?, isHaveStock : Boolean) : Boolean {
        ...
        if(name.isEmpty()){
            ...
            state.value = ProductCreateEditState.Validate(weight = "Berat tak boleh kosong")
            return false
        }
        return true
    }
